...
function render() {
    ...
    for (let i = 0; i < points.length; i++) {
        const p = points[i];
        const value = getNextValue(2 * p.x - mouse.x, 2 * p.y - mouse.y);
...
function getNextValue(x, y) {
    ...
    const x1 = Math.sin(a) - Math.cos(b * x) * scalar;
    const y1 = Math.sin(c * x) - Math.cos(d * y) * -scalar;

    return Math.atan2(y1 - y, x1 - x) * scalar;
}
